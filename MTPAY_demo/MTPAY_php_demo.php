...
class MTPAY{
    private $app_id = "94090357"; //商户帐号ID (APPID)<由 MTPAY 分配,在app-API配置(pc商户后台--账户中心--商户信息) 中获取>
    private $mch_no = "3555"; //商户号 (UID)<由 MTPAY 分配,在app-API配置(pc商户后台--账户中心--商户信息) 中获取>
    private $app_key ='9B26EA1A57AEDAB6B65AC9ADE4660D0C';//密钥
    private $into_url = "https://merchant-api.mtpay.io/api/recharge/check/v2";  //入金地址
	private $rmb_price_url = "https://merchant-api.mtpay.io/api/recharge/convert/v1";//查询报价
    private $order_query_url = "https://merchant-api.mtpay.io/api/recharge/order/query"; //查询订单状态
	private $withdrawal_url = "https://merchant-api.mtpay.io/api/recharge/customer/withdrawal"; //法币出金申请
    private $withdrawal_history_url = "https://merchant-api.mtpay.io/api/recharge/customer/withdrawal/history"; //法币入金申请历史
    private $withdrawal_delete_url = "https://merchant-api.mtpay.io/api/recharge/customer/withdrawal/delete"; //删除未审核的法币出金申请记录

//curl -X GET -H "content-type:application/json" -H "access_key:8A42FADDD2397F4841B1F85C2C13094CAB152151" -H "app_id:41043343" https://merchant-api.mtpay.io/api/recharge/convert/v1?timestamp=1609385362&p1=9357&p2=CNY&p3=695
    ...
    public function pay($closeCurrency,$amount,$realName){
	   ...
	   $str = $amount.'&'.$this->mch_no.'&'.$orderNo.'&'.$timestamp;
        ...
        $rlt = $this->httpGET($url,$str);
        var_dump($rlt);
        //{"code":200,"data":{"url":"https://checkout.mtpay.io/?token=ilC5D8PMKVk%3D#/"},"message":"success"}
    ...
    function httpGET($url,$str) {
        ...
        echo 'url:'.$url."</br>";    //请求地址和参数：https://merchant-api.mtpay.io/api/recharge/check/v2?p1=100&p2=11082429&p3=1553155910&timestamp=1553155910
